angular.module('uifordocker.filters', [])
    ...
    .filter('runstopbtn_text', function () {
        ...
        return function (text) {
            if (text === '' || text === 'Created') {
                return 'Start Pipeline';   // start button
            }
            else if (text.indexOf('Exit') !== -1 && text !== 'Exit 0') {
                return 'Start Pipeline';
            }
            else {
                return 'Stop pipeline';    // stop button
            }
        };
    })
                ...
                if (image.RepoTags && image.RepoTags.length > 0) {
                    ...
                    if (tag_size>1) {
                        if (tag === '<none>:<none>') {
                            ...
                            var tmp = tag.split(':')[1];
                            if (tmp === "RNA_Seq_dui_paired_end") {
                                return "RNA-Seq (paired-end)";
                            }
                            else if (tmp === "ChIP_Seq_dui_single_end") {
                                return "ChIP-Seq (single-end)";
                            }
                            else if (tmp === "ChIP_Seq_dui_paired_end") {
                                return "ChIP-Seq (paired-end)";
                            }
                            else {
                                return tmp;
                            }
    ...
    .filter('get_baseInstance', function () {
        return function(container) {
            var inst_name = container.Names[0];
            if ( inst_name.includes('RNA_Seq_paired-end') ) {
                return 'RNA-Seq (paired-end)'
            }
            else if ( inst_name.includes('ChIP_Seq_single-end') ) {
                return 'ChIP-Seq (single-end)'
            }
